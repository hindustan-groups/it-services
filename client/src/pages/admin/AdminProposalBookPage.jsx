 ...
 * 
 * 9-Page Premium Corporate Proposal & Quotation Book Studio
 * Dual-Pane Editor + Live Scalable A4 Document Viewer
 * 
 * Specs:
 * - Exact A4 Dimensions: 210mm x 297mm @media print
 * - Zero Text Clipping, Zero Page Cutoff
 * - Dual Pane UI: Left Form Controls (380px) + Right A4 Canvas Viewer
 * - Zoom Controls: 60%, 75%, 90%, 100%
 */
...
export default function AdminProposalBookPage() {
  ...
  // UI State
  const [zoomScale, setZoomScale] = useState(0.75) // Default 75% for comfortable viewing
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white flex flex-col print:bg-white print:text-black">
      <SEO title="Proposal Studio — Admin" />

      {/* Inject Exact Print CSS Rules */}
      <style>{`
        @media print {
          @page {
            size: A4 portrait;
      ...
      <header className="print-hidden bg-slate-950 border-b border-slate-800 px-4 py-3 sticky top-0 z-50 shadow-xl">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          ...
          <div className="flex items-center gap-2 bg-slate-900 px-3 py-1 rounded-xl border border-slate-800">
            <span className="text-[10px] font-bold text-slate-400 uppercase mr-1">Preview Zoom:</span>
            <button
              type="button"
            ...
            <span className="font-mono text-xs font-bold text-[#0A3D91] bg-white px-2 py-0.5 rounded">
      ...
      <div className="flex-1 flex overflow-hidden">
              ...
              {[
                ...
                <button
                  ...
                  className={`py-2.5 transition-colors ${
                    activeFormTab === t.id
                      ? 'bg-[#0A3D91] text-white border-b-2 border-[#D32F2F]'
            ...
            <div className="p-4 space-y-4 text-xs">
              ...
              {activeFormTab === 'client' && (
                <div className="space-y-3">
                  ...
                  <div>
                    <label className="text-[10px] font-extrabold text-slate-400 uppercase block mb-1">Client Organization Name</label>
                    <input
                      type="text"
                      value={proposalData.clientCompany}
                      onChange={(e) => setProposalData({ ...proposalData, clientCompany: e.target.value })}
                      className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-1.5 text-white focus:outline-none focus:border-[#0A3D91]"
                    />
                  </div>

                  <div>
                    <label className="text-[10px] font-extrabold text-slate-400 uppercase block mb-1">Contact Person Name</label>
                    <input
                      type="text"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-1.5 text-white font-mono text-red-400"
                    />
                  </div>

                  <div>
                    <label className="text-[10px] font-extrabold text-slate-400 uppercase block mb-1">Client Email</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="text-[10px] font-extrabold text-slate-400 uppercase block mb-1">Client Phone</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button
                        ...
                        className="text-[10px] bg-emerald-600/20 text-emerald-300 px-2 py-0.5 rounded border border-emerald-500/30 hover:bg-emerald-600/40"
                      >
                        + Add Item
                      </button>
                    </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="text-[10px] bg-blue-600/20 text-blue-300 px-2 py-0.5 rounded border border-blue-500/30 hover:bg-blue-600/40"
                    >
                      + Add Item
                    </button>
                  </div>
                  ...
                  <div className="space-y-2">
                    {proposalData.lineItems.map((item, idx) => (
                      <div key={item.id} className="p-2 bg-slate-900 border border-slate-800 rounded-lg space-y-1.5">
                        <div className="flex items-center justify-between">
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-800">
                    ...
                    <div>
                      <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">GST (%)</label>
                      <input
                        type="text"
                        value={proposalData.gstPercent}
                        onChange={(e) => setProposalData({ ...proposalData, gstPercent: e.target.value })}
                        className="w-full bg-slate-900 border border-slate-800 rounded px-2.5 py-1 text-white font-mono text-center"
                      />
                    </div>
                  </div>
                </div>
              ...
              {activeFormTab === 'presets' && (
                <div className="space-y-3">
                  <h4 className="font-extrabold text-slate-200 text-xs">Load Package Presets</h4>
                  <button
                    type="button"
        ...
        {/* RIGHT LIVE SCALABLE A4 VIEWPORT CANVAS */}
        <main className="flex-1 bg-slate-900 p-6 overflow-auto flex justify-center items-start">
          ...
          >
            ...
            {/* ─────────────────────────────────────────────────────────────────
                ...
                ───────────────────────────────────────────────────────────────── */}
            <div className="a4-page bg-white text-black shadow-2xl rounded-sm p-8 flex flex-col justify-between relative overflow-hidden w-[210mm] min-h-[296.8mm] border border-gray-200">
              ...
              <div className="relative z-10">
                ...
                <div className="grid grid-cols-12 gap-5 my-6 items-start">
                  ...
                  <div className="col-span-7 bg-[#F2F6FA] border border-[#D0DFEF] rounded-2xl p-5 shadow-xs">
                    ...
                    <h3 className="font-heading font-black text-lg text-[#0B2545] border-b-2 border-[#D32F2F] pb-1 inline-block mb-3">
                  ...
                  <div className="col-span-5 space-y-3">
                    <div className="p-3.5 rounded-xl border border-gray-200 bg-white shadow-xs flex items-center gap-3">
                      <div className="w-9 h-9 rounded-lg bg-[#0B2545]/10 text-[#0B2545] flex items-center justify-center shrink-0">
                        <FileText className="w-4 h-4" />
                      </div>
                      <div>
                        <span className="text-[9px] font-extrabold text-gray-400 uppercase block">
                          QUOTATION NO.
                        </span>
                ...
                {/* Prepared By Section */}
                <div className="my-6 pt-3 border-t border-gray-200 max-w-sm">
                  <span className="text-[9px] font-extrabold text-gray-400 uppercase tracking-widest block mb-1">
                    PREPARED BY
                  </span>
                  <h4 className="font-heading font-black text-xs text-[#0B2545]">
                    HINDUSTAN PROJECTS IT SERVICES
                  </h4>
                  <div className="space-y-0.5 text-[10px] text-gray-600 mt-1.5 font-medium">
                ...
                <div className="grid grid-cols-3 gap-2 border-t-2 border-gray-200 pt-3">
                  <div className="p-2 rounded-xl border border-gray-200 bg-slate-50 flex items-center gap-2">
                    <div className="w-6 h-6 rounded bg-[#0B2545] text-white flex items-center justify-center text-[10px] shrink-0">
                      🖥️
                    </div>
                    <div>
                      <span className="font-heading font-black text-[10px] text-[#0B2545] uppercase block leading-tight">
                        WEBSITE
                      </span>
                      <span className="text-[8px] text-gray-500 font-bold block">DEVELOPMENT</span>
                    </div>
                  </div>
                  ...
                  <div className="p-2 rounded-xl border border-gray-200 bg-slate-50 flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <span className="text-[8px] text-gray-500 font-bold block">SOLUTIONS</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-4 bg-[#0B2545] text-white text-[9px] py-1.5 px-3 rounded-b-lg flex items-center justify-between font-mono relative z-10">
                <div className="flex items-center gap-2">
            ...
            <div className="a4-page bg-white text-black shadow-2xl rounded-sm p-8 flex flex-col justify-between relative overflow-hidden w-[210mm] min-h-[296.8mm] border border-gray-200">
              <div>
                ...
                <h2 className="font-heading text-xl font-black text-[#0A3D91] mb-4 flex items-center gap-2">
                  <Building2 className="w-5 h-5 text-[#D32F2F]" />
                  01. Client Information &amp; Project Overview
                </h2>

                <div className="mb-6">
                  <h3 className="font-heading text-xs font-extrabold text-[#0A3D91] uppercase tracking-wider mb-2">
                    Client Profile Details
                  </h3>
                  <table className="w-full text-xs text-[#333333] border border-gray-200 rounded-lg overflow-hidden">
                    <tbody>
                      <tr className="border-b border-gray-200 bg-slate-50">
                        <td className="p-2.5 font-bold text-[#0A3D91] w-1/3 border-r border-gray-200">Organization Name</td>
                        <td className="p-2.5 font-semibold">{proposalData.clientCompany}</td>
                      </tr>
                      <tr className="border-b border-gray-200">
                        <td className="p-2.5 font-bold text-[#0A3D91] border-r border-gray-200">Contact Representative</td>
                        <td className="p-2.5 font-semibold">{proposalData.contactPerson} ({proposalData.clientDesignation})</td>
                      </tr>
                      <tr className="border-b border-gray-200 bg-slate-50">
                        <td className="p-2.5 font-bold text-[#0A3D91] border-r border-gray-200">Email Address</td>
                        <td className="p-2.5 font-semibold">{proposalData.clientEmail}</td>
                      </tr>
                      <tr className="border-b border-gray-200">
                        <td className="p-2.5 font-bold text-[#0A3D91] border-r border-gray-200">Phone Number</td>
                        <td className="p-2.5 font-semibold">{proposalData.clientPhone}</td>
                      </tr>
                      <tr>
                        <td className="p-2.5 font-bold text-[#0A3D91] border-r border-gray-200">Corporate Address</td>
                ...
                <div className="space-y-4">
                  <div className="p-4 rounded-xl bg-slate-50 border border-slate-200">
                    <h3 className="font-heading text-xs font-extrabold text-[#0A3D91] uppercase tracking-wider mb-1.5">
                  ...
                  <div className="p-4 rounded-xl border border-[#0A3D91]/20 bg-[#0A3D91]/5">
                    <h3 className="font-heading text-xs font-extrabold text-[#0A3D91] uppercase tracking-wider mb-2">
                      Key Strategic Objectives
                    </h3>
                    <ul className="space-y-1.5 text-xs text-[#333333]">
                      <li className="flex items-start gap-2">
                        <CheckCircle2 className="w-3.5 h-3.5 text-[#0A3D91] shrink-0 mt-0.5" />
            ...
            <div className="a4-page bg-white text-black shadow-2xl rounded-sm p-8 flex flex-col justify-between relative overflow-hidden w-[210mm] min-h-[296.8mm] border border-gray-200">
              <div>
                ...
                <div className="mb-5">
                  <h3 className="font-heading text-xs font-extrabold text-[#0A3D91] uppercase tracking-wider mb-2 flex items-center gap-1.5">
            ...
            <div className="a4-page bg-white text-black shadow-2xl rounded-sm p-8 flex flex-col justify-between relative overflow-hidden w-[210mm] min-h-[296.8mm] border border-gray-200">
              <div>
                ...
                <h2 className="font-heading text-xl font-black text-[#0A3D91] mb-5 flex items-center gap-2">
                  <Cpu className="w-5 h-5 text-[#D32F2F]" />
                  03. Technical Features &amp; Architecture
                </h2>

                <div className="grid grid-cols-2 gap-3.5 text-xs text-[#333333]">
                  {[
                    ...
                    return (
                      <div key={idx} className="p-3.5 rounded-xl border border-gray-200 bg-slate-50 flex items-start gap-3">
                        <div className="w-9 h-9 rounded-lg bg-[#0A3D91] text-white flex items-center justify-center shrink-0">
                          <IconC className="w-4.5 h-4.5" />
                        </div>
                        <div>
                          <h3 className="font-heading font-black text-xs text-[#0A3D91] mb-0.5">{feat.title}</h3>
                          <p className="text-gray-600 text-[10px] leading-relaxed">{feat.desc}</p>
                        </div>
                      </div>
                    )
                  })}
              ...
              <div>
                ...
                <div className="space-y-3 text-xs text-[#333333]">
                  {proposalData.timelinePhases.map((item, idx) => (
                    <div key={idx} className="p-3.5 rounded-xl border border-gray-200 bg-slate-50 flex items-start gap-3">
                      <div className="px-2.5 py-0.5 rounded bg-[#0A3D91] text-white font-mono font-bold text-[10px] shrink-0">
                        {item.phase}
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-0.5">
                          <h3 className="font-heading font-black text-xs text-[#0A3D91]">{item.title}</h3>
                        ...
                        <p className="text-gray-600 text-[10px] leading-relaxed">{item.desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="a4-page bg-white text-black shadow-2xl rounded-sm p-8 flex flex-col justify-between relative overflow-hidden w-[210mm] min-h-[296.8mm] border border-gray-200">
              <div>
                ...
                <table className="w-full text-xs text-[#333333] border border-gray-200 rounded-lg overflow-hidden mb-5">
                  <thead>
                  ...
                  <tbody>
                    {proposalData.lineItems.map((item, idx) => (
                      <tr key={item.id || idx} className={`border-b border-gray-200 ${idx % 2 === 1 ? 'bg-slate-50' : ''}`}>
                        ...
                        <td className="p-2.5 font-bold text-[#0A3D91]">{item.desc}</td>
                        <td className="p-2.5 text-center font-semibold">{item.qty}</td>
                        <td className={`p-2.5 text-right font-bold ${item.rate === 'INCLUDED' ? 'text-emerald-600' : 'text-gray-900'}`}>
                ...
                <div className="flex justify-end mb-5">
                  <div className="w-64 bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-xs space-y-1.5">
                    ...
                    <div className="flex justify-between text-sm font-black text-[#0A3D91] pt-1.5 border-t-2 border-[#0A3D91]">
                ...
                <div className="p-3.5 rounded-xl bg-[#0A3D91]/5 border border-[#0A3D91]/20 text-xs">
                  <h4 className="font-heading font-black text-[#0A3D91] uppercase mb-1">Commercial Payment Terms</h4>
                  <p className="text-gray-700 leading-relaxed font-semibold text-[11px]">{proposalData.paymentTerms}</p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-3.5 text-xs text-[#333333]">
                  {proposalData.legalTerms.map((clause, idx) => (
                    <div key={clause.id || idx} className="p-3 rounded-xl border border-gray-200 bg-slate-50">
                      <h3 className="font-heading font-extrabold text-[#0A3D91] text-[11px] mb-0.5">{clause.title}</h3>
                      <p className="text-gray-600 text-[10px] leading-relaxed">{clause.text}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="a4-page bg-white text-black shadow-2xl rounded-sm p-8 flex flex-col justify-between relative overflow-hidden w-[210mm] min-h-[296.8mm] border border-gray-200">
              <div>
                ...
                <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 text-xs text-[#333333] mb-8">
                ...
                <div className="grid grid-cols-2 gap-6 text-xs text-[#333333] mt-6">
                  <div className="p-5 rounded-xl border-2 border-dashed border-gray-300 bg-slate-50 flex flex-col justify-between h-56">
                    <div>
                      <span className="text-[9px] font-extrabold text-gray-400 uppercase tracking-wider block mb-1">
                        CLIENT AUTHORIZED SIGNATORY
                      </span>
                      <h3 className="font-heading font-black text-xs text-[#0A3D91]">{proposalData.clientCompany}</h3>
                    </div>

                    <div className="border-t border-gray-300 pt-2">
                      <p className="font-bold text-[11px]">Authorized Signature &amp; Stamp</p>
                      <p className="text-gray-500 text-[10px] mt-0.5">Name: {proposalData.contactPerson}</p>
                      <p className="text-gray-500 text-[10px]">Designation: {proposalData.clientDesignation}</p>
                      <p className="text-gray-500 text-[10px]">Date: ____________________</p>
                    </div>
                  </div>

                  <div className="p-5 rounded-xl border-2 border-[#0A3D91] bg-[#0A3D91]/5 flex flex-col justify-between h-56">
                    <div>
                      <span className="text-[9px] font-extrabold text-[#0A3D91] uppercase tracking-wider block mb-1">
                        VENDOR AUTHORIZED SIGNATORY
                      </span>
                      <h3 className="font-heading font-black text-xs text-[#0A3D91]">Hindustan Projects IT Services</h3>
                    </div>

                    <div className="my-auto self-center">
                      <div className="w-16 h-16 rounded-full border-2 border-[#0A3D91] border-dashed flex items-center justify-center text-[#0A3D91] font-mono text-[8px] font-black text-center p-1 uppercase">
                        OFFICIAL SEAL &amp; STAMP
                      </div>
                    </div>

                    <div className="border-t border-[#0A3D91]/30 pt-2">
                      <p className="font-bold text-[#0A3D91] text-[11px]">Authorized Signatory</p>
                      <p className="text-gray-600 text-[10px] mt-0.5">Designation: Managing Director</p>
                      <p className="text-gray-600 text-[10px]">Date: {proposalData.date}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="a4-page bg-white text-black shadow-2xl rounded-sm p-8 flex flex-col justify-between relative overflow-hidden w-[210mm] min-h-[296.8mm] border border-gray-200">
              <div>
                ...
                <div className="text-center my-10 max-w-xl mx-auto">
                  <span className="inline-block px-3 py-1 rounded-full bg-[#0A3D91]/10 text-[#0A3D91] font-mono text-xs font-bold uppercase tracking-widest mb-3">
                    THANK YOU FOR YOUR TRUST
                  </span>
                  <h2 className="font-heading text-3xl font-black text-[#0A3D91] tracking-tight">
                ...
                <div className="p-6 rounded-2xl bg-[#0A3D91] text-white my-6 max-w-xl mx-auto shadow-xl">
                  <div className="grid grid-cols-3 gap-4 items-center">
                    ...
                    <div className="text-center bg-white p-2.5 rounded-xl text-gray-900">
                      <div className="w-20 h-20 mx-auto bg-slate-100 border border-slate-300 rounded flex items-center justify-center">
                        <QrCode className="w-14 h-14 text-[#0A3D91]" />
                      </div>
                      <span className="text-[8px] font-extrabold uppercase block mt-1 text-[#0A3D91]">
                        SCAN TO VERIFY
                      </span>
                    </div>
                  </div>
